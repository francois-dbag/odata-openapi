...
namespace Company.Function
{
    public static class HttpTrigger1
    {
        ...
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = null)] HttpRequest req, ILogger log)
        {
            ...
            if (v2toV4xsl == null)
            {
                log.LogInformation("First run on this host - caching stylesheets and transforms from " + Directory.GetCurrentDirectory() + Path.PathSeparator);

                v2toV4xsl = new XslCompiledTransform();
                v2toV4xsl.Load(Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar + "V2-to-V4-CSDL.xsl");

                v4cdsltoopenapixsl = new XslCompiledTransform();
                v4cdsltoopenapixsl.Load(Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar + "V4-CSDL-to-OpenAPI.xsl"); 

                csdltoodataversion = new XslCompiledTransform();
                csdltoodataversion.Load(Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar + "OData-Version.xsl"); 
